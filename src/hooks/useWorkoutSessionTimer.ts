...
export function useWorkoutSessionTimer() {
  ...
  const elapsedSeconds = useMemo(() => stored ? Math.max(0, Math.floor((now - stored.startedAt) / 1000)) : 0, [stored, now]);

  return { status: stored ? 'running' as const : 'idle' as const, elapsedSeconds, start, stop };
}
