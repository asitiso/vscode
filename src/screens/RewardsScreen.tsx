...
function getNextGrowthLabel(count: number, starLevel: number): string {
  if (starLevel >= 4) return '최대 성장 완료';
  const nextStar = (starLevel + 1) as 2 | 3 | 4;
  const remaining = Math.max(0, STAR_THRESHOLDS[nextStar] - count);
  return `${nextStar}성까지 ${remaining}장 남음`;
}
...
export function RewardsScreen() {
  ...
  const totalReps = todayEntries.reduce(
    (sum, entry) => sum + (entry.reps ?? 0) * Math.max(1, entry.sets ?? 1),
    0,
  );
  const latestLog = todayLogs.at(-1);
  const mostActiveExercise = todayEntries.reduce<Record<string, number>>((counts, entry) => {
    counts[entry.exerciseId] = (counts[entry.exerciseId] ?? 0) + (entry.sets ?? entry.durationMinutes ?? 1);
    return counts;
  }, {});
  ...
  const todayCards = state.grantedPacks
    ...
    .map((pack) => {
      const card = pack.resultCardId ? CARDS_BY_ID[pack.resultCardId] : undefined;
      const owned = card ? state.ownedCards[card.id] : undefined;
      return card && owned ? { pack, card, owned } : null;
    })
