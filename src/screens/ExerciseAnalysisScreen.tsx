...
export function ExerciseAnalysisScreen({
  exerciseId,
  workoutLogs,
  onBack,
}: {
  ...
  if (!analysis) {
    return (
      <div className="exercise-analysis-screen exercise-analysis-screen--empty">
        <button type="button" className="exercise-analysis__back" onClick={onBack}>← 운동 리포트</button>
        <div><span>📊</span><strong>아직 분석할 기록이 없어요</strong><p>운동을 기록하면 상세 분석이 표시됩니다.</p></div>
      </div>
    );
  }
  ...
  return (
    <div className="exercise-analysis-screen">
      <header className="exercise-analysis__header">
        <button type="button" className="exercise-analysis__back" onClick={onBack}>← 운동 리포트</button>
        <span>EXERCISE ANALYSIS</span>
        <h1>{analysis.exerciseName}</h1>
        <p>최근 운동일 {analysis.latestDate}</p>
      </header>
      ...
      <section className="exercise-analysis__section">
        <div className="exercise-section-heading"><strong>누적 통계</strong><span>전체 기록 기준</span></div>
        <div className="exercise-total-grid">
          <div><strong>{analysis.totals.activeDays}</strong><span>운동일</span></div>
          <div><strong>{analysis.totals.recordCount}</strong><span>기록 횟수</span></div>
          <div><strong>{analysis.totals.sets}</strong><span>총 세트</span></div>
          <div><strong>{analysis.totals.reps}</strong><span>총 반복</span></div>
          <div><strong>{analysis.totals.durationMinutes}</strong><span>총 시간(분)</span></div>
        </div>
      </section>
      ...
      <section className="exercise-analysis__section">
        <div className="exercise-section-heading"><strong>최근 기록</strong><span>최근 4회</span></div>
        <div className="exercise-recent-list">
          {analysis.recentRecords.map((record) => (
            <article key={record.id}>
              <div><strong>{record.date}</strong><span>{FEELING_LABELS[record.feeling]}</span></div>
              <p>{recordMetrics(record)}</p>
              {record.note && <small>{record.note}</small>}
            </article>
          ))}
        </div>
        {!showAll && <button className="exercise-analysis__all" type="button" onClick={() => setShowAll(true)}>전체 기록 보기</button>}
      </section>

      {showAll && <ExerciseHistoryList records={analysis.history} workoutLogs={workoutLogs} onSelectDay={setSelectedDay} />}
      <WorkoutDayDetailSheet report={selectedDay} onClose={() => setSelectedDay(null)} onSelectExercise={() => undefined} />
    </div>
  );
}
